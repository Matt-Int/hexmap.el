...
Optionally provide the FILL, STROKE, and LABEL for the hex."
  (let ((fill (or fill "transparent"))
    ...
    (svg-text svg label :x x :y (+ y (* size 0.8)) :font-size (/ size 10) :text-anchor "middle" :stroke "black")))
...
Provide BIOME to get the matching `biome-highlight-colours'."
  (let ((x1 x)
	...
	(colour (if biome (cdr (assoc biome biome-highlight-colours)) "white")))
    (svg-line svg x y2 x y :stroke-color colour :stroke-width (* size (/ 3.0 80.0)))
    (svg-polygon svg `((,(+ x (* size 0.2)) . ,(- y (* size 0.15)))
		       (,x . ,(- y2 (* size 0.2)))
...
Provide BIOME to get the matching `biome-highlight-colours'."
  (let ((draw-function (if (or (eq biome 'taiga) (eq biome 'arctic))
    ...
    (apply draw-function `(,svg ,(- x (* size 0.5 )) ,(+ y (* size 0.2 )) ,size ,biome))
    (apply draw-function `(,svg ,(+ x (* size 0.34)) ,(+ y (* size 0.15)) ,size ,biome)))
)
